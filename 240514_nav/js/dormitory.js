...
const setPage = (page) => {
    ...
    if (page === 1) {
        ...
        if (storedReservations) {
            ...
            reservations.map((reservation) => reservation.date = new Date(reservation.date)); // .date에 저장된 string을 -> Date 객체로 바꾸자
        } else { // 저장된 예약들이 없으면, (아예 예약 완료 버튼 안눌렀을 떄, 처음..)
            reservations = []
        }
        ...
        // 세탁기 번호, 시간 보관하자
        newReservation.washingmachine = washingmachineSelect.value; // 세탁기 option들 에서 사용자가 선택한 세탁기와 value 속성값을 가져오자
...
const initWashingmachineTime = () => {
    ...
    reservations.forEach((reservation) => {
        // 사용자가 예약한 날짜와 지금 입력하고 있는 새로운 예약의 날짜가 같으면, 그 세탁기 번호의 그 시간을 빼자 (== 상관 없음)
        if (reservation.date.getFullYear() == newReservation.date.getFullYear() && reservation.date.getMonth() == newReservation.date.getMonth() && reservation.date.getDate() == newReservation.date.getDate()) {
            const { wasingmachine, time } = reservation;
            const index = allWashingmachineTime[washingmachine].indexOf(String(time));  //1 -> "1"
            if (index > -1) {   //예약된 시간 찾았다면
                allWashingmachineTime[washingmachine].splice(index, 1); // 그 시간 빼자
            }
...
const saveReservations = () => {
    // 원래는 백엔드에 reservations 정보를 넘겨서 데이터베이스에 저장해야 함
    // 그냥 로컬에 기록해둘 것이에요. localStorage 라는 친구
    alert("예약 완료");
    // key와 우리가 얻고 싶은 자료
